 ...
 *
 * Out-of-tree fork of Zephyr's drivers/video/imx219.c, taken so the register
 * programming can be corrected against Raspberry Pi's kernel driver
 * (drivers/media/i2c/imx219.c) while bringing up the ESP32-P4 CSI-2 receiver.
 * It binds the same "sony,imx219" compatible, so select exactly one of
 * CONFIG_VIDEO_IMX219 (in-tree) and CONFIG_VIDEO_IMX219_OOT (this file).
 *
 * Divergences from the in-tree driver are each marked "OOT FIX:".
 */
...
static const int64_t imx219_2dl_link_frequency[] = {
 ...
 * driver advertises.
 *
 * The in-tree table used PLL_OP_MPY = 50 with PREPLLCK_OP_DIV = 3 and
 * OPSYCK_DIV = 1, i.e. 24 MHz / 3 * 50 = 400 Mbps per lane, while
 * VIDEO_CID_LINK_FREQ advertises 456 MHz (= 912 Mbps per lane). Any CSI
 * receiver that trusts the advertised value - as it must, that being the only
 * thing exported - programs its D-PHY for a frequency range the link is
 * nowhere near. PLL_OP_MPY = 114 gives 24 / 3 * 114 = 912 Mbps, which matches.
 *
 * PLL_VT_MPY and VTPXCK_DIV come from the same kernel table (57 and 5); the
 * in-tree values of 30/4 were part of the "tuned for 1920x1080" approximation
 * that its own comment flags as provisional.
 */
/* clang-format off */
	...
	[IMX219_RAW8_FULL_FRAME] =
		{
			...
			.width_min = 4,
			.width_max = IMX219_FULL_WIDTH,
			.width_step = 4,
			.height_min = 4,
			.height_max = IMX219_FULL_HEIGHT,
			.height_step = 4,
		},
	[IMX219_RAW10_FULL_FRAME] =
		{
...
static const uint32_t imx219_framerates[] = {
	[IMX219_30FPS_IDX] = IMX219_30FPS,
	[IMX219_15FPS_IDX] = IMX219_15FPS,
};

static int imx219_enum_frmival(const struct device *dev, struct video_frmival_enum *fie)
{
	if (fie->index >= ARRAY_SIZE(imx219_framerates)) {
		return -EINVAL;
	}
...
static int imx219_set_stream(const struct device *dev, bool on, enum video_buf_type type)
{
	...
	ret = video_write_cci_reg(&cfg->i2c, IMX219_CCI_MODE_SELECT,
...
static DEVICE_API(video, imx219_driver_api) = {
...
static int imx219_init_ctrls(const struct device *dev)
{
	...
	ret = video_init_ctrl(&ctrls->exposure, dev, VIDEO_CID_EXPOSURE,
			      (struct video_ctrl_range){.min = 0x0000,
							.max = 0xffff,
							.step = 1,
	...
	ret = video_init_ctrl(
		...
		(struct video_ctrl_range){
			.min = 0x00, .max = 0x3ff, .step = 1, .def = IMX219_DT_PEDESTAL_DEFAULT});
	if (ret < 0) {
		return ret;
	}

	ret = video_init_ctrl(&ctrls->analogue_gain, dev, VIDEO_CID_ANALOGUE_GAIN,
			      (struct video_ctrl_range){.min = IMX219_ANALOG_GAIN_MIN,
							.max = IMX219_ANALOG_GAIN_MAX,
							.step = 1,
	...
	ret = video_init_ctrl(
		...
		(struct video_ctrl_range){
			.min = 0x000, .max = 0xfff, .step = 1, .def = IMX219_DIGITAL_GAIN_DEFAULT});
	if (ret < 0) {
		return ret;
	}

	ret = video_init_int_menu_ctrl(&ctrls->linkfreq, dev, VIDEO_CID_LINK_FREQ, 0,
				       imx219_2dl_link_frequency,
 ...
 * transmitting" from "the receiver cannot see it". Everything else in this
 * bring-up is inferred; these are the sensor's own registers.
 */
static int cmd_imx219_regs(const struct shell *sh, size_t argc, char **argv)
{
	static const struct i2c_dt_spec i2c = I2C_DT_SPEC_GET(DT_NODELABEL(imx219));
	static const struct {
		const char *name;
		uint32_t reg;
		const char *note;
	} regs[] = {
		{"CHIP_ID", IMX219_CCI_CHIP_ID, "expect 0x0219"},
		{"MODE_SELECT", IMX219_CCI_MODE_SELECT, "1 = streaming, 0 = standby"},
		{"CSI_LANE_MODE", IMX219_CCI_CSI_LANE_MODE, "1 = 2-lane"},
		{"EXCK_FREQ", IMX219_CCI_EXCK_FREQ, "MHz << 8, expect 0x1800"},
...
SHELL_CMD_REGISTER(imx219_regs, NULL, "Read back IMX219 configuration over I2C", cmd_imx219_regs);
#endif /* CONFIG_SHELL */
