...
{
	zassert_equal(dxl_table_lookup((enum dxl_control)9999, NULL, NULL), -EINVAL,
		      "out-of-range returns -EINVAL");
}

ZTEST(dynamixel_tables, test_lookup_null_outparams)
{
	zassert_ok(dxl_table_lookup(MODEL_NUMBER, NULL, NULL), "NULL outparams allowed");
}
